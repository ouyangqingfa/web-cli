<!--左侧大菜单-->
<template>
    <a-menu mode="inline" theme="dark" v-model:openKeys="openKeys" v-model:selectedKeys="selectedKeys" @click="onMenuItemClick">
...
watchEffect(() => {
    openKeys.value = route.matched.map((a) => a.name?.toString() ?? "") ?? []; //[route.matched[1]?.name?.toString() ?? ""]
