...
export function hexToRgb(sColor: string): [number, number, number] {
    ...
    if (sColor && reg.test(sColor)) {
        ...
        //处理六位的颜色值
        return [parseInt("0x" + sColor.slice(1, 3)), parseInt("0x" + sColor.slice(3, 5)), parseInt("0x" + sColor.slice(5, 7))];
    } else {
        return [0, 0, 0];
    }
}
